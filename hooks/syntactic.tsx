            ...
            if (variables[varName]) {
                errorVars.push(
                    findErrorLine(
                        txt,
                        varName,
                        "Variable ya definida en la linea: "
                    )
                );
                continue;
            }
        ...
        if (
                ...
                if (variables && !variables[name]) {
                    errors.push(
                        findErrorLine(
                            txt,
                            varName,
                            "Variable sin definir en la linea: "
                        )
                    );
                }
            });
            if (aritmetics) {
                ...
                for (const varName of variablesUsed) {
                    ...
                    if (
                        ...
                        errors.push(`Variable ${name} no es de tipo Num`);
                        continue;
                    }
                }
            }
            ...
            errors.push(
                findErrorLine(
                    txt,
                    tokens[i].value,
                    "Declaracion invalida en la linea: "
                )
            );
            continue;
        }
...
const validateCode = (code: string) => {
    ...
    if (result) {
        ...
        errorVars.push(
            "Falta la(s) expresiones init, vars, body or end, recuerda usar el orden correcto"
        );
    }
...
export const validate = (code: any) => {
    ...
    if (errors.length === 0) {
        console.log("Validación sintáctica exitosa");
        return null;
    } else {
        console.log(errors);
